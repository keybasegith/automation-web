...
export const metadata = pageMetadata("/",
  "Independent Wealth Management in Canada | Keybase Financial Group",
  "Explore independent wealth management and financial planning in Canada with Keybase Financial Group. Find an advisor for your financial goals.");
...
/**
 * The carousel runs the firm's own Perspectives pieces rather than market
 * commentary: the band is evergreen ("reshaping your financial future"), and a
 * dated CPI reading would age badly sitting in it for months. An article with
 * no artwork of its own is skipped — the slide is half photograph.
 */
const CAROUSEL_CATEGORY = "Perspectives";

export default async function Home() {
  const published = await getPublishedArticles();
  const featured = published.slice(0, FEATURED_COUNT);
  const carousel = published
    .filter((a) => a.category === CAROUSEL_CATEGORY && a.heroImage)
    .map((a) => ({
      category: a.category,
      title: a.card?.title ?? a.title,
  ...
  return (
    <div className="font-franklin min-h-screen bg-white text-[#1a2433]">
      ...
      <main>
        ...
        {/* ---------- Keybase Answer ---------- */}
        {/* Renders nothing when KEYBASE_ANSWER_ENABLED is false. */}
        ...
        <section id="insights" className="bg-white">
          <div className="mx-auto grid max-w-[1280px] gap-12 px-5 py-24 sm:px-8 sm:py-28 lg:grid-cols-[minmax(0,360px)_1fr] lg:gap-20">
            ...
            {/* Every row is a real article with a page behind it. Nothing is
                listed here that a reader cannot open. */}
            <div>
              {featured.map((post, i) => (
        ...
        <section id="growth-calculator" className="border-t border-black/10 bg-[#f7f9fa]">
          <div className="mx-auto max-w-[1280px] px-5 py-10 sm:px-8 sm:py-12">
            ...
            <Reveal delay={120} className="mt-6">
              <CompoundInterestCalculator compact headingLevel="h3" />
            </Reveal>
          </div>
        </section>

        {/* ---------- Featured Carousel ---------- */}
        <FeaturedCarousel slides={carousel} />
      </main>
